...
struct lttng_event_desc {
	...
	unsigned int nr_fields;
	struct module *owner;
	void *event_notifier_callback;		/* notify probe */
};
...
struct lttng_event_notifier_enabler {
	...
	/* FIXME: head list of struct lttng_ust_filter_bytecode_node */
...
static inline int lttng_syscalls_unregister_event_notifier(struct lttng_event_notifier_group *group)
{
	return 0;
}

static inline int lttng_syscall_filter_enable_event_notifier(struct lttng_event_notifier_group *group,
		const char *name)
{
	return -ENOSYS;
}

static inline int lttng_syscall_filter_disable_event_notifier(struct lttng_event_notifier_group *group,
		const char *name)
{
	return -ENOSYS;
}
